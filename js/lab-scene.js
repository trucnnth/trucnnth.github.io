...
(function() {
  ...
  // Trucnnth — 45-degree bow facing right
  function drawTrucnnthBowR(cx, cy, pct) {
    const p = P();
    // Phases: stand(0-0.15) → bow(0.15-0.85) → stand(0.85-1)
    const bowing = pct > 0.15 && pct < 0.85;
    if (!bowing) { drawTrucnnth(cx, cy); return; }
    // Lower body stays in place
    ctx.fillStyle = p.tPants;
    ctx.fillRect(cx, cy + 7, 4, 2);
    ctx.fillStyle = '#483020';
    ctx.fillRect(cx, cy + 9, 2, 2);
    ctx.fillRect(cx + 2, cy + 9, 2, 2);
    // Upper body tilted 45° — shifted right 2px and down 2px
    ctx.fillStyle = p.tShirt;
  ...
  // Trucnnth — 45-degree bow facing left
  function drawTrucnnthBowL(cx, cy, pct) {
    const p = P();
    const bowing = pct > 0.15 && pct < 0.85;
    if (!bowing) { drawTrucnnthLeft(cx, cy); return; }
    // Lower body stays in place
    ctx.fillStyle = p.tPants;
    ctx.fillRect(cx, cy + 7, 4, 2);
    ctx.fillStyle = '#483020';
    ctx.fillRect(cx, cy + 9, 2, 2);
    ctx.fillRect(cx + 2, cy + 9, 2, 2);
    // Upper body tilted 45° — shifted left 2px and down 2px
    ctx.fillStyle = p.tShirt;
    ctx.fillRect(cx - 1, cy + 5, 4, 2);
    // Pouch
    ctx.fillStyle = p.tPouchStrap;
    ...
    ctx.fillStyle = p.tPouch;
    ctx.fillRect(cx + 3, cy + 7, 2, 1);
    // Head tilted forward (left) and down
    ctx.fillStyle = p.lHair;
  ...
  // Leey — facing LEFT (mirrored)
  function drawLeeyLeft(cx, cy, blink) {
    const p = P();
    ctx.fillStyle = p.lHair;
    ctx.fillRect(cx, cy, 4, 2);
    ctx.fillStyle = p.skin;
  ...
  function draw() {
    ...
    const uzayTalkX = 160;         // 4px right of uzay (152+4+4)
    const wsStandX = wsX + 10;     // 168 — at workstation
    const leeyTalkX = 178;         // 13px left of leey (191)
    ...
    t += Math.round(48/S);  const t7 = t;  // walk to uzay (48px)
    t += 12;  const t8 = t;  // talk uzay
    t += Math.round(8/S);   const t9 = t;  // walk to workstation (8px)
    t += 35;  const t10 = t; // work at workstation
    t += Math.round(20/S);  const t11 = t; // walk to leey (20px)
    t += 22;  const t12 = t; // bow to leey (longer)
    t += Math.round(40/S);  const t13 = t; // walk to goralski (40px)
    ...
    t += Math.round(44/S);  const t19 = t; // walk scanner to leey (222-178=44px)
    t += 22;  const t19b = t; // bow to leey again
    t += Math.round(10/S);  const t19c = t; // walk leey to ws (178-168=10px)
    ...
    const states = [
      ...
      [t3,  koikoiTalkX,   koikoiTalkX,   'talk-r',  'koikoi'],
      ...
      [t19,  scannerX,      leeyTalkX,     'walk-l',  null],
      [t19b, leeyTalkX,    leeyTalkX,     'bow-r',   'leey'],
      [t19c, leeyTalkX,    wsStandX,      'walk-l',  null],
    ...
    if (!isDark()) {
      ...
      drawLeeyLeft(wsX + 33, groundY - 11, blinkL);
      ...
      if (stype === 'work-r') {
        ...
        drawTrucnnthBowR(tx, groundY - 11, pct);
      } else if (stype === 'bow-l') {
        drawTrucnnthBowL(tx, groundY - 11, pct);
